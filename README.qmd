...
system("sed -i 's/^signature/#signature/' template.qmd")
fs::file_delete(c("csic.png","ipp.png","europe_age_csic_ipp.png","sigfile.png"))
...
# CSIC Letter Format Template

This is a Quarto template that assists you in creating a letter on IPP-CSIC letterhead. There are two versions of the template, one using LaTeX (`letter-pdf`), and one using Typst (`letter-typst`). The latter is faster to render.

This template is a fork from  Rob J Hyndman's [Monash Letter Format Template](https://github.com/quarto-monash/letter)
